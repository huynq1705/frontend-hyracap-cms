...
const PAGE_ACTIVE_IMPORT: string[] = ["customer", "service-list", "products"];
...
export default function TopTableCustom(props: TopTableCustomProps) {
  ...
  return (
    ...
    >
      ...
      >
        {check_actions.includes("schedule") && (
          <ButtonCore
            title="Lấy mã đặt hẹn online"
            type="borderColor"
            onClick={() => {
              actions?.getLink
                ? actions.getLink()
                : alert("not exist getLink fn");
            }}
          />
        )}
        {check_actions.includes("schedule") && (
          <ButtonCore
            ...
            type="borderColor"
            onClick={() => {
              navigate(`/admin/config-schedule`);
            }}
          />
        )}

        {actions?.downloadFn && PAGE_ACTIVE_EXPORT.includes(check_actions) && (
          <ButtonCore
        ...
        {actions?.uploadFn && PAGE_ACTIVE_IMPORT.includes(check_actions) && (
          <ButtonCore
        ...
        {children && children}
        {actions?.createFn && !PAGE_OFF_CREATE.includes(check_actions) && (
          <ButtonCore
