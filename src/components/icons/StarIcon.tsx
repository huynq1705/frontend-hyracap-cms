...
interface StarIconProps extends React.SVGProps<SVGSVGElement> {
    complete: string; // Yêu cầu giá trị chuỗi đại diện cho phần trăm (ví dụ: "30%")
    size ?: number
}

function StarIcon(props: StarIconProps) {
    const { complete , size = 22 } = props;
    const id = uuidv4();
    return (
        <svg
            width={size}
        ...
        >
            {/* Định nghĩa gradient từ trái sang phải */}
            <defs>
                <linearGradient id={id} x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset={complete} stopColor="#F79009" />
                    <stop offset={complete} stopColor="#D0D5DD" />
                    <stop offset="100%" stopColor="#D0D5DD" />
                </linearGradient>
            </defs>
            {/* Sử dụng gradient trong path */}
            <path
                ...
                fill={`url(#${id})`} // Sử dụng gradient đã định nghĩa
            />
        </svg>
    );
}
