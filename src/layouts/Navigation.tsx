...
const Navigation = (props: NavigationProps): JSX.Element => {
  ...
  const navTopDefine: NavList[] = useMemo(() => {
    ...
    //phân quyền
    const filterNavTopDefine: NavList[] = filteredNavItems
    .map((group) => ({
      ...group,
      items: group.items
        .filter(itemTo =>
          Array.isArray(itemTo.role)
            ? itemTo.role.some(role =>
              userInfo?.role?.role_permission.some((permission: any) =>
      ...
      .map((item) => ({
        ...
        children: item.children?.filter((child) =>
          child.role ? userInfo?.role?.role_permission.includes(child.role[0]) : true
        ),
      }))
    
    }))
    return filterNavTopDefine;
  }, [userInfo?.role.role_permission, T]);
