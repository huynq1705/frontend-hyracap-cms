...
const Header = (props: HeaderProps): JSX.Element => {
  ...
  const findCustomer = () =>{
    getCustomer({text : keySearch,page : 1 ,take : 999}).then((e)=>{
  ...
  return (
    <Box className="flex justify-between items-center w-full space-x-1 flex-wrap gap-3" sx={{
      flexDirection: {
        xs: ""
      }
    }}>
          ...
          <Box className="flex-row justify-center items-center relative header-search-box gap-3 flex-1 hidden md:block">
            ...
            <div
              className={clsx(
                ...
                openDropdown && (keySearch !== "" || listCustomer != null) && "!flex",
              )}
            >
              { (listCustomer && listCustomer.length) > 0 &&
                listCustomer.map((x : any) => (
                  <div
                    ...
                    onClick={() => navigate(`/admin/customer/view/${x.id}`)}
                  >
