      ...
      {dataConvert.map((item: any, index: any) => (
        ...
        >
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            ...
            <div className="text-gray-9 text-base py-1">
              {formatCurrency(+item?.direct_bonus)}
            </div>
          </div>
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            <span className="font-medium text-gray-9 text-sm">
            ...
            <div className="text-gray-9 text-base py-1">
              {formatCurrency(+item?.manage_bonus)}
            </div>
          </div>
    ...
    },
    {
      ...
      render: (_: any, d: any) => (
        <Stack direction={"column"} spacing={1}>
          ...
          >
            {d?.staff_position?.position?.name}
          </Typography>
        </Stack>
      ),
    },
    ...
    },
    {
      ...
      render: (_: any, d: any) => (
        <Stack direction={"column"} spacing={1}>
          ...
          >
            {formatCurrency(+d?.direct_bonus)}
          </Typography>
        </Stack>
      ),
    },
    {
      title: "Thưởng quản lý",
      dataIndex: "manage_bonus",
      width: 120,
      render: (_: any, d: any) => (
        <Stack direction={"column"} spacing={1}>
          <Typography
...
const SaleHistoryTable = (props: SaleHistoryTableProps) => {
  ...
  useEffect(() => {
    if (data?.data) {
      setTotal({
        ...
        total_kpi:
          data?.data.reduce((sum, item) => sum + (+item.kpi_bonus || 0), 0) ||
          0,
      });
    }
  }, [data]);
  ...
  const handleRowClick = (record: any) => {
    ...
    navigate(
      `/admin/sale_history/${record.id}?effective_from__gt=${date}&effective_from__lt=${endDate}`
    );
  };
