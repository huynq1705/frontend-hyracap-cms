    ...
    const columns: any = [
        ...
        },
        {
            ...
            render: (_: any, d: any) => (
                ...
                >
                    <CStatus
                        type={d?.status === 0 ? "error" : "success"}
                        name={d?.status === 0 ? "Chưa duyệt" : "Đã duyệt"}
                    />
                </Stack>
            ),
        },
                            ...
                            >
                                {hasPermission.getDetail && (
                                    <ActionButton
                                        ...
                                        onClick={() => {
                                            navigate(
                                                `/admin/withdrawRequest/view/${d?.id}`
                                            );
                                            actions.togglePopup("edit");
                                        }}
                                    />
...
const CTable = (props: CTableProps) => {
    ...
    const { data, isLoading, isError, refetch } = useQuery({
        queryKey: ["GET_WITHDRAW", param_payload, pathname],
        queryFn: () => getWithdrawRequest(param_payload),
    ...
    useEffect(() => {
        ...
        if (pathname.includes("view") && !popup.edit) {
            navigate(`/admin/withdrawRequest/view/${code}`);
            togglePopup("edit");
        }
        if (pathname.includes("edit") && !popup.edit) {
            navigate(`/admin/withdrawRequest/edit/${code}`);
            togglePopup("edit");
        }
