  ...
  const columns: any = [
      ...
      render: (_: any, item: any, index: number) => (
        <Stack direction={"column"} spacing={1}>
          <Typography
            ...
            onClick={() => {
              navigate(`/admin/users/view/${item?.user?.sub}`);
              actions.togglePopup("edit");
            }}
          >
      ...
      render: (_: any, item: any, index: number) => (
        <Stack direction={"column"} spacing={1}>
          <Typography
            ...
            onClick={() => {
              navigate(
                `/admin/sale_history/${item?.staff_id}?effective_from__gt=${date}&effective_from__lt=${endDate}`
              );
            }}
          >
    ...
    },
    {
      ...
      render: (_: any, item: any) => (
        ...
        >
          {item?.effective_from && item?.effective_to
            ? `${formatDate(item.effective_from, "DDMMYYYY")} - ${formatDate(
                item.effective_to,
                "DDMMYYYY"
              )}`
            : "Hợp đồng chưa có hiệu lực"}
