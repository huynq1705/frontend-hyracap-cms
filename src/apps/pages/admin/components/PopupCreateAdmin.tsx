...
function PopupCreateAdmin(props: PopupConfirmRemoveProps) {
  ...
  const [listRole] = React.useState<OptionSelect>([{ label: "Admin", value: "Admin" }, { label: "Quản lý", value: "Quản lý" }]);
      ...
      switch (response) {
        case true: {
          ...
          dispatch(
            setGlobalNoti({
              type: "success",
              message: T("create") + " " + t("manage") + " " + t("success"),
            }),
          );
          break;
        }
        case false: {
          dispatch(
            setGlobalNoti({
              type: "error",
              message: T("create") + " " + t("manage") + " " + t("fail"),
            }),
          );
  ...
  const handleSubmitUpdate = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await putAccount(formData, formData.id, KEY_REQUIRED, fileList);
      switch (response) {
        case true: {
          // navigate("/customer");
          refetch();
          ...
          dispatch(
            setGlobalNoti({
              type: "success",
              message: T("update") + " " + t("manage") + " " + t("success"),
            }),
          );
          break;
        }
        case false: {
          dispatch(
            setGlobalNoti({
              type: "error",
              message: T("update") + " "+ t("manage") + " "+ t("fail"),
            }),
          );
          break;
        }
  ...
  const handleSubmitPass = async () => {
    setIsLoading(true)
    try {
      const response = await putAccountChangPass({ new_password: formData.password_config, old_password: formData.password }, formData.id, ["new_password", "old_password"])
      switch (response) {
        case true: {
          // navigate("/customer");
          handleClose();
    ...
    <>
      ...
      >
        <Box>
          ...
          >
            <h2 style={{ fontSize: 20, color: palette.textQuaternary }}>{T(statusPage) + " admin"}</h2>
            <button
              onClick={handleClose}
          ...
          {editPassword ?
            <Stack
              ...
              <h3 style={{ color: palette.textQuaternary }}>Thông tin tài khoản</h3>
              <div className="wrapper-from">
                <MyTextField
                  label={T("userName")}
                  errors={errors}
                ...
                {statusPage !== "create" ?
                  <Box
                    ...
                    pt={"66px"}
                    width={"calc(50% - 12px)"}>
                    <ButtonCore type="bgWhite" title="Đổi mật khẩu" onClick={() => setEditPassword(false)} />
                  </Box>
                  : <Box width={"40%"} />}
                {
                  statusPage === "create" &&
          ...
          >
            ...
            {
              (statusPage === "detail") ?
                <ButtonCore title={"Đóng"}
                  onClick={handleClose} />
                :
                <ButtonCore title={"Hoàn tất"}
