...
import { Typography, Table, PaginationProps, Tooltip, Empty } from "antd";
    ...
    <div className=" flex md:hidden flex-col space-y-4">
      {dataConvert.map((item: any, index: any) => (
        ...
        >
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            ...
            <div className="text-gray-9 text-base py-1">
              <span className="font-medium" style={{ color: "#50945d" }}>
                {item?.order_detail?.order?.customer?.full_name .length > 20
                  ? item?.order_detail?.order?.customer?.full_name .slice(0, 30) + "..."
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            <span className="font-medium text-gray-9 text-sm">Loại thẻ</span>
            <div className="text-gray-9 text-base py-1">
              <span>{item?.order_detail.prepaid_card_face_value?.name || "- -"}</span>
            </div>
          </div>
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            ...
            <div className="text-gray-9 text-base py-1">
              <span>{`${formatCurrency(item?.amount_used ?? 0,)}`}</span>
            </div>
          </div>
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            ...
            <div className="text-gray-9 text-base py-1">
              <span>
                {formatCurrency(
                  item?.order_detail?.prepaid_card_face_value?.denominations || 0
                )}
              </span>
            </div>
          </div>
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            ...
            <div className="text-gray-9 text-base py-1">
              <span>{formatDate(item?.expiration_date, "DDMMYYYY") ?? "--"}</span>
            </div>
          </div>
        </div>
      ))}
...
const getColumns = (props: ColumnProps) => {
  ...
  const columns: any = [
    ...
    },
    {
      ...
      width: 300,
      render: (_: any, d: any) => {
        const tooltip = d?.order_detail?.order?.customer?.full_name || "- -";
        const content =
    ...
    },
    {
      ...
      render: (_: any, d: any) => (
        <Stack direction={"column"} spacing={1}>
          ...
          >
            {d?.order_detail.prepaid_card_face_value?.name || "- -"}
          </Typography>
        </Stack>
      ),
    },
    {
      ...
      dataIndex: "paid",
      render: (_: any, d: any) => (
        <Typography.Text>{`${formatCurrency(
          d?.amount_used ?? 0,
        )}`}</Typography.Text>
      ),
    },

    {
      ...
      render: (_: any, d: any) => (
        <Typography.Text>
          {formatCurrency(
            d?.order_detail?.prepaid_card_face_value?.denominations || 0,
          )}
        </Typography.Text>
      ),
    },
...
const CTable = () => {
  ...
  const { getPrepaidCardFace } = apiPrepaidCardFaceService();
  const handleSearch = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => getPrepaidCardFace(param_payload),
