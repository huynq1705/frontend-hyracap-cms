...
export interface PrepaidHistoryProps {}

export default function PrepaidHistory(props: PrepaidHistoryProps) {
  const { code } = useParams();
  const { getHistoryService } = apiOrderService();
  const [prepaidHistory, setPrepaidHistory] = React.useState<any>([]);
  ...
  const init = async () => {
    try {
      const data = await getHistoryService(
        `/order-detail-prepaid-card/payment/history?customer_id=${code}`,
      );
      if (data.statusCode == 200 && data.data.table)
        setPrepaidHistory(data.data.table);
  ...
  return (
    <div className="px-4">
      ...
      >
        <div className="flex gap-4 flex-wrap">
          ...
          <StatusCardV2
            statusData={{
              label: "Tổng số dư:",
              value: formatCurrency(statistical.total_remaining_balance),
              color: "#0D63F3",
            }}
            hightLine="label"
          />
          <StatusCardV2
            statusData={{
              ...
              value: formatCurrency(statistical.available_balance),
              color: "#50945D",
            }}
            hightLine="label"
          />
          <StatusCardV2
            statusData={{
              label: "Số dư hết hạn:",
              value: formatCurrency(statistical.expired_balance),
              color: "#D83D32",
            }}
            hightLine="label"
          />
      ...
      >
        {/* <CustomCardList dataConvert={prepaidHistory} /> */}
        {prepaidHistory.length < 1 && (
          <Empty
