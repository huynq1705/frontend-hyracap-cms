    ...
    <div className=" flex md:hidden flex-col space-y-4">
      {dataConvert.map((item: any, index: any) => (
        ...
        >
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            <span className="font-medium text-gray-9 text-sm">Quản lý</span>
            <div className="text-gray-9 text-base py-1">
              <span>
                {item?.leader
                ...
                <div className="flex items-center g-8 justify-start space-x-4">
                  {hasPermission.getDetail && (
                    <ActionButton
                      type="view"
                      onClick={() => {
                        navigate(`/admin/group_sale_history/view/${item?.id}`);
              ...
              >
                {hasPermission.getDetail && (
                  <ActionButton
                    type="view"
                    onClick={() => {
                      navigate(`/admin/group_sale_history/view/${d?.id}`);
                      actions.togglePopup("edit");
                    }}
                  />
                )}
  ...
  useEffect(() => {
    if (data?.data) {
      setTotal({
        ...
        total_kpi:
          data?.data.reduce((sum, item) => sum + (+item.kpi || 0), 0) || 0,
        total_sales_revenue:
          data?.data.reduce(
          ...
          <div className="flex justify-between items-center">
            <div className="wrapper-from">
              ...
              <StatusCardV2
                statusData={{
                  label: "Tổng kpi",
                  value: `${formatCurrencyNoUnit(+total.total_kpi) ?? 0} vnđ`,
                  color: "#7A52DE",
                }}
                customCss="min-w-[250px]"
              />
              <StatusCardV2
                statusData={{
                  ...
                  value: `${
                    formatCurrencyNoUnit(+total.total_sales_revenue) ?? 0
                  }  vnđ`,
                  color: "#7A52DE",
                }}
                customCss="min-w-[250px]"
