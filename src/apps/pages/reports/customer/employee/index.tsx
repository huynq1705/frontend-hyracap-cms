      ...
      {dataConvert.map((item: any, index: any) => (
        ...
        >
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            ...
            <div className="text-gray-9 text-base py-1">
              <span className="font-medium" style={{ color: "#50945d" }}>
                {item?.account?.full_name.length > 20 ?
                  item?.account?.full_name.slice(0, 30) + "..." :
                  item?.account?.full_name ? item?.account?.full_name :
                    "- -"}
              </span>
            </div>
          </div>
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            ...
            <div className="text-gray-9 text-base py-1">
              <span>{item?.account?.position ?? "- -"}</span>
            </div>
          </div>
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (_: any, item: any) => {
        const tooltip = item?.account?.full_name ?? "- -";
        const content =
...
const EmployeePage = () => {
  ...
  const handleGetParam = () => {
    ...
    if (!params["account_id"]) params["account_id"] = code;
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["GET_ACCOUNT_ORDER", param_payload, pathname],
    queryFn: () => getAccountOrder(param_payload),
  ...
  useEffect(() => {
    initData();
    setFormData((prev) => ({ ...prev, ["person_in_charge"]: [code] }));
  }, []);
  ...
  const handleOnchange = (e: any, isSelectOne?: boolean) => {
    ...
    if (name === "person_in_charge") {
      let new_value = value;
      searchParams.set("account_id", new_value);
      setSearchParams(searchParams);
  ...
  const arrayButtonExport = [
    {
      label: "Báo cáo đánh giá nhân viên",
      fn: () => {
        setPopup(true);
        setContent("báo cáo đánh giá nhân viên");
      },
    },
  ];
  const handleOnConfirm = () => {
    setPopup(false);
    downloadExcelFile('account-order/export', param_payload)
  }
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="flex items-center gap-3 py-3 px-5 text-sm bg-white">
        ...
        <span>{"Báo cáo đánh giá nhân viên chi tiết" ?? "- -"}</span>
      </div>
      <div className="flex flex-wrap items-start gap-3 sm:flex-row justify-between py-6 px-4 ">
        <h2 >Đánh giá nhân viên {dataConvert[0]?.account?.full_name ?? "- -"}</h2>
        <div
