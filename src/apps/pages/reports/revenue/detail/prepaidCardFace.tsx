...
const CTablePrepaidCard
  = () => {
    ...
    const { getReportPrepaidCard } = apiReportService();
    const { data, isLoading, isError, refetch } = useQuery({
      queryKey: ["GET_REPORT_PREPAID_CARD", param_payload, pathname],
      queryFn: () => getReportPrepaidCard(param_payload),
    ...
    const dataConvert = useMemo(() => {
      ...
      if (data_res)
        return data_res.list_prepaid_card;
      return [];
    }, [data]);
    ...
    const arrayButtonExport = [
      {
        label: "Báo cáo doanh thu thẻ liệu trình",
        fn: () => {
          setPopup(true);
          setContent("báo cáo doanh thu thẻ liệu trình");
        },
      },
    ];
    const handleOnConfirm = () => {
      setPopup(false);
      downloadExcelFile('report/revenue/prepaid-card/export', param_payload)
    }
    return (
      <>
        <div className="flex items-center gap-3 py-3 px-5 text-sm bg-white">
          ...
          <span>Chi tiết báo cáo thẻ trả trước</span>
        </div>
            ...
            <div className="wrapper-from flex-wrap flex gap-4 items-center">
              <StatusCardV2
                statusData={{
                  ...
                  value: `${data && data?.data?.prepaid_card?.total_payment}`,
                  color: "#7A52DE",
                }}
              ...
              <StatusCardV2
                statusData={{
                  ...
                  value: `${data && formatCurrencyNoUnit(data?.data?.prepaid_card?.total_amount_paid)} vnđ`,
                  color: "#217732",
                }}
                customCss="min-w-[250px]"
              />
...
export default CTablePrepaidCard
  ;
