      ...
      {dataConvert.map((item: any, index: any) => (
        ...
        >
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            <span className="font-medium text-gray-9 text-sm">Tiêu đề</span>
            <div className="text-gray-9 text-base py-1">
              <p
          ...
          <div className="border-b border-t-0 border-x-0 border-solid border-gray-4 last:border-none animate-fadeup  px-3 py-2">
            <span className="font-medium text-gray-9 text-sm">Người tạo</span>
            <div className="text-gray-9 text-base py-1">
              <span>
              ...
              <div className="text-gray-9 text-base py-1">
                <div className="flex items-center g-8 justify-start space-x-4">
                  {hasPermission.getDetail && (
                    <ActionButton
                      type="view"
                      onClick={() => navigate(`/admin/blog/view/${item.id}`)}
                    />
                  )}
        ...
        <>
          ...
          >
            {hasPermission.getDetail && (
              <ActionButton
                type="view"
                onClick={() => navigate(`/admin/blog/view/${d.id}`)}
                className="!h-5"
              />
            )}
            {hasPermission.update && (
              <ActionButton
                type="edit"
                onClick={() => navigate(`/admin/blog/edit/${d.id}`)}
                className="!h-5"
              />
            )}
            {hasPermission.delete && (
...
const ListBlog = (props: ListRequestDepositProps) => {
  ...
  useEffect(() => {
    setKeySearch({
      ...keySearch,
      created_at__between: key_search?.created_at__between || "",
    });
  }, [keySearch?.created_at]);
    ...
    <>
      <Box width="100%" className="custom-table-wrapper w-full ">
        <div className="w-full flex-wrap md:flex items-start justify-start gap-4 ">
          ...
          <SearchBoxTable
            keySearch={keySearch?.title__ilike || ""}
            setKeySearch={(e) => {
              setKeySearch({ ...keySearch, title__ilike: e });
            }}
